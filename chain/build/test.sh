...
# start backend server
make PROJECT_DIR="$P_DIR"/backend PROJECT_NAME="$(basename "$P_DIR"/middle)" BASE_DIR="$P_DIR"/middle/dist -f build/Makefile start
...
make PROJECT_DIR="$P_DIR"/middle PROJECT_NAME="$(basename "$P_DIR")"/middle BASE_DIR="$P_DIR"/middle/dist \
    CONF_CONSUMER_FILE_PATH="$P_DIR"/middle/conf/test.yml -f build/Makefile integration
if [ $result -eq 0 ]; then
    result=$?
fi
